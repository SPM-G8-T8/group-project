...
def get_all_applicants(db: db_dependency):
    res = db.query(models.RoleApplications).filter(
...
def get_applicants(role_listing_id: int, db: db_dependency):
    res = (
        ...
        .join(
            ...
            models.StaffDetails.staff_id == models.RoleApplications.staff_id,
        )
        .filter(
            models.RoleApplications.role_listing_id == role_listing_id,
            models.RoleApplications.role_app_status == "applied",
        )
    )
    if res.count() == 0:
...
def create_application(application: RoleApplicationCreate, db: db_dependency):
    ...
    res = db.query(models.RoleApplications).filter(
        models.RoleApplications.role_listing_id == application.role_listing_id,
        models.RoleApplications.staff_id == application.staff_id,
    )
